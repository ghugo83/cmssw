...
PixelTelescopeSpecialSeedGenerator::PixelTelescopeSpecialSeedGenerator(const edm::ParameterSet& conf): 
  conf_(conf),
...

{
  	...
  	edm::LogVerbatim("PixelTelescopeSpecialSeedGenerator") << "Constructing PixelTelescopeSpecialSeedGenerator";
...
PixelTelescopeSpecialSeedGenerator::~PixelTelescopeSpecialSeedGenerator(){
    if (theRegionProducer) { delete theRegionProducer; theRegionProducer = nullptr; }
}

void PixelTelescopeSpecialSeedGenerator::endRun(edm::Run const&, edm::EventSetup const&){
    if (theSeedBuilder)    { delete theSeedBuilder;    theSeedBuilder = nullptr; }
}

void PixelTelescopeSpecialSeedGenerator::beginRun(edm::Run const&, const edm::EventSetup& iSetup){
	std::string builderName = conf_.getParameter<std::string>("TTRHBuilder");
        ...
        edm::LogVerbatim("PixelTelescopeSpecialSeedGenerator") << "Initializing...";
	if (useScintillatorsConstraint){
        	edm::ParameterSet upperScintPar = conf_.getParameter<edm::ParameterSet>("UpperScintillatorParameters");
          	...
          	edm::LogVerbatim("PixelTelescopeSpecialSeedGenerator")
	...
	for (iPSet = pSets.begin(); iPSet != pSets.end(); iPSet++){
		...
		if (navigationDirection == "insideOut") theNavDirs.push_back(insideOut);
        	...
        	edm::LogVerbatim("PixelTelescopeSpecialSeedGenerator") << "hitsGenerator done";
	} 
	...
	theSeedBuilder = new SeedGeneratorForPixelTelescope(theMagfield.product(), 
							  theTracker.product(), 
							  theBuilder.product(),
...
void PixelTelescopeSpecialSeedGenerator::produce(edm::Event& e, const edm::EventSetup& iSetup)
{
  // get Inputs
  ...
  edm::LogVerbatim("PixelTelescopeSpecialSeedGenerator") << " number of seeds = "<< output->size();
  e.put(std::move(output));
}

bool PixelTelescopeSpecialSeedGenerator::run(const edm::EventSetup& iSetup,
					       const edm::Event& e,
...
bool PixelTelescopeSpecialSeedGenerator::buildSeeds(const edm::EventSetup& iSetup,
					 const edm::Event& e,
  ...
  edm::LogInfo("PixelTelescopeSpecialSeedGenerator")<<"osh.size() " << osh.size();
  for (unsigned int i = 0; i < osh.size(); i++){
	...
	if (preliminaryCheck(shs,iSetup)){
		...
		for (std::vector<TrajectorySeed*>::const_iterator iSeed = seeds.begin(); iSeed != seeds.end(); iSeed++){
		  if (!*iSeed) {edm::LogError("PixelTelescopeSpecialSeedGenerator")<<"a seed pointer is null. skipping.";continue;}
			if (postCheck(**iSeed)){
				output.push_back(**iSeed);
			}
			delete *iSeed;
			edm::LogVerbatim("PixelTelescopeSpecialSeedGenerator") << "Seed built";
		}
	}
  }	 
...
bool PixelTelescopeSpecialSeedGenerator::preliminaryCheck(const SeedingHitSet& shs, const edm::EventSetup &es ){
...
bool PixelTelescopeSpecialSeedGenerator::postCheck(const TrajectorySeed& seed){
	...
	StraightLinePlaneCrossing planeCrossingLower( Basic3DVector<float>(state->position()), 
	  ...
	  planeCrossingLower.position(*lowerScintillator);
        if (!(positionUpper.first && positionLower.first)) {
                 edm::LogVerbatim("PixelTelescopeSpecialSeedGenerator::checkDirection") 
