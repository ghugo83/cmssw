  ...
  thePositiveRingsComps(posRings.begin(),posRings.end())
{
...
#ifdef EDM_ML_DEBUG
  //--------- DEBUG INFO --------------
  LogDebug("TkDetLayers") << "==== DEBUG Phase2ITtiltedBarrelLayer =====" ; 
  LogTrace("TkDetLayers") << "Phase2ITtiltedBarrelLayer Cyl r,lenght: "
                          << theCylinder->radius() << " , "
  ...
  for (vector<const GeometricSearchDet*>::const_iterator i=theNegativeRingsComps.begin();
    ...
    LogTrace("TkDetLayers") << "negative rings in Phase2IT tilted barrel pos z,perp,eta,phi: " 
			    << (**i).position().z()    << " , " 
			    << (**i).position().perp() << " , " 
			    << (**i).position().eta()  << " , " 
			    << (**i).position().phi()  ;
  ...
  for (vector<const GeometricSearchDet*>::const_iterator i=PixelBarrelLayer::components().begin();
       i != PixelBarrelLayer::components().end(); i++){
    LogTrace("TkDetLayers") << "rods in Phase2IT tilted barrel pos z,perp,eta,phi: " 
			    << (**i).position().z()    << " , " 
			    << (**i).position().perp() << " , " 
			    << (**i).position().eta()  << " , " 
			    << (**i).position().phi()  ;
  ...
  for (vector<const GeometricSearchDet*>::const_iterator i=thePositiveRingsComps.begin();
    ...
    LogTrace("TkDetLayers") << "positive rings in Phase2IT tilted barrel pos z,perp,eta,phi: " 
			    << (**i).position().z()    << " , " 
			    << (**i).position().perp() << " , " 
			    << (**i).position().eta()  << " , " 
			    << (**i).position().phi()  ;
  }
  LogTrace("TkDetLayers") << "==== end DEBUG Phase2ITtiltedBarrelLayer =====" ; 
  //----------------------------------- 
#endif
}

Phase2ITtiltedBarrelLayer::~Phase2ITtiltedBarrelLayer(){
...
void 
Phase2ITtiltedBarrelLayer::groupedCompatibleDetsV( const TrajectoryStateOnSurface& tsos,
  ...
  PixelBarrelLayer::groupedCompatibleDetsV(tsos, prop, est, closestResultRods);
...
#ifdef EDM_ML_DEBUG
  LogDebug("TkDetLayers") << "==== output di Phase2ITtiltedBarrelLayer =====" ; 
