...
#include "Geometry/TrackerNumberingBuilder/plugins/CmsTrackerITRingBuilder.h"
...
void CmsTrackerITLayerBuilder::buildComponent(DDFilteredView& fv, GeometricDet* g, std::string s){

  LogTrace("DetConstruction") << " CmsTrackerITLayerBuilder::buildComponent ";
  CmsTrackerLadderBuilder theCmsTrackerLadderBuilder;
  CmsTrackerITRingBuilder theCmsTrackerITRingBuilder;
  ...
  case GeometricDet::panel:
    theCmsTrackerITRingBuilder.build( fv, subdet, s );
    break;
  default:
    edm::LogError("CmsTrackerITLayerBuilder")<<" ERROR - I was expecting a ladder or a panel, I got a "<<ExtractStringFromDDD::getString(s,&fv);
  }  
  g->addComponent(subdet);

}

void CmsTrackerITLayerBuilder::sortNS(DDFilteredView& fv, GeometricDet* det){
  ...
  for(uint32_t i=0; i<comp.size();i++){
    ...
    if(component->type()== GeometricDet::ladder){
      ...
      edm::LogError("CmsTrackerITLayerBuilder")<<"ERROR - wrong SubDet to sort..... "<<det->components().front()->type();
    }
  }
